---
title: "来做一道RSA入门题吧!"
excerpt: "ezRSA"
date: "2025-12-09"
slug: "rsa-example"
section: "crypto"
---

<div class="intro-article">

# **相当于我的题解**😂😂

先给出题目:

```python
//cipher.py
from Crypto.Util.number import * 
from gmpy2 import * 
with open("/home/ctf/flag", "r") as f: 
flag = f.read().strip().encode() 
m = bytes_to_long(flag) 
p = getPrime(512) 
q = next_prime(p) 
n = p * q 
e = 0x10001 
d = inverse(e, (p-1) * (q-1)) 
c = pow(m, e, n) 
print(f"c = {c}") 
print(f"d = {d}") 
c =  
d = 
```

## 分析题目逻辑:

1️⃣ 泄露了私钥指数 d

在 RSA 中，有：

> ed≡1(modφ(n))

因此必然存在整数 k，使得：

> ed−1=k⋅φ(n)

这意味着我们可以通过 k = e*d - 1 来构造所有可能的 φ(n)。

2️⃣ p 和 q 是**相邻素数**

本题中：

> q = next_prime(p)

因此：

p、q 极为接近

q - p = g 很小（一般 < 1000）

这使得 φ(n) 的形式：

> φ(n)=(p−1)(q−1)

可以展开成一个关于 p 的二次方程，枚举 g 就能把 p 解出来。

## 🧠 解题整体思路

✔ Step 1：根据 d 求出 φ(n) 候选

计算：

> k=ed−1

因为：

> k=t⋅φ(n)

我们只需要枚举 k 的 小因子 t：

> φ=k/t

即可得到所有 φ(n) 的可能值。

✔ Step 2：利用 q = next_prime(p) 求 p

已知：

q = p + g   （g 很小）

φ = (p-1)(p+g-1)

展开：

> 𝑝^2+(𝑔−2)𝑝+(1-𝑔)−𝜑=0

这是一个标准二次方程。

对每个 g（0~2000）：

1.解方程得到 p

2.判断 p 是否为素数

3.q 是否等于 next_prime(p)

4.是否满足 φ = (p-1)(q-1)

若全部符合 => 成功分解 n。

✔ Step 3：解密密文

得到 p、q 后：

> n = p * q

> m = pow(c, d, n)

> flag = long_to_bytes(m)

即可得到最终 flag!😜😜

## 最终脚本☺️

```python
//exploit.py
from Crypto.Util.number import *
from gmpy2 import is_prime, iroot, next_prime
#见题目
c = 2382475417612221029195991422974637682790907959505872057612...
d = 2587033034158764455750626942590486200509176127009824643...
e = 65537

k = e*d - 1

# 枚举小因子
for t in range(1, 20000):
    if k % t != 0:
        continue

    phi = k // t

    # 枚举 g = q - p
    for g in range(1, 2000):
        # 二次方程: p^2 + (g-2)p + (1-g) - phi = 0
        a = 1
        b = g - 2
        c2 = (1 - g) - phi

        # 求 Δ
        D = b*b - 4*a*c2
        if D < 0:
            continue

        r, ok = iroot(D, 2)
        if not ok:
            continue

        # 求解 p
        p = (-b + r) // 2
        if p <= 0:
            continue

        if not is_prime(p):
            continue

        q = next_prime(p)
        if (p-1)*(q-1) != phi:
            continue

        print("[+] Found p =", p)
        print("[+] Found q =", q)

        n = p*q
        m = pow(c, d, n)
        print(long_to_bytes(m))
        exit()
```

得到:

> XSWCTF{YOu_KnOW_H0w_To_f4CTOrIZe_PHl}

good!😍😍😍

## 补充解法:**Fermat 分解**

当：

> q−p 很小

就可以用 费马分解法(Fermat 分解)：

> n=a^2−b^2=(a−b)(a+b)

其中：

> a=⌈sqrt(n)​⌉

不会装latex插件，见谅😭😭😭😭

且:

> b^2=a^2−n

如果：

> b^2=k^2

则：

> b=k

并得到：

> p=a−b,q=a+b

完成分解。

如果b^2不是完全平方数：

> a←a+1

回到 Step 2循环

**通常很快就能得到结果...**

可以自行搜索资料...☺️

代码实现:

```python
exploit.py
import gmpy2

def fermat(n):
    a = gmpy2.isqrt(n)
    if a * a < n:
        a += 1

    while True:
        b2 = a * a - n
        b = gmpy2.isqrt(b2)
        if b * b == b2:
            return a - b, a + b
        a += 1
```

*that is end*☺️

感谢阅读!☺️

</div>